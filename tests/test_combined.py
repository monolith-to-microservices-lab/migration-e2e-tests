...
def test_user_and_linked_sale_both_propagate_with_ids_preserved(run_id):
    user_resp = httpx.post(f"{MONOLITH_URL}/users", json={"name": f"{run_id}_combined_user"}, timeout=10)
    ...
    sale_row = poll_for_row(
        SALES_DSN, "SELECT id, user_id FROM sales WHERE id = %s", (sale_id,)
    )
