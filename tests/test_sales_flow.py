...
def test_sale_create_update_delete_propagates_end_to_end(evidence, run_id):
    ...
    t0 = time.time()
    resp = httpx.post(
        f"{MONOLITH_URL}/sales", json={"user_id": 1, "item_name": item_name, "quantity": 3}, timeout=10
    )
    assert resp.status_code == 201, resp.text
    sale_id = resp.json()["id"]

    row = poll_for_row(SALES_DSN, "SELECT id, item_name, quantity FROM sales WHERE id = %s", (sale_id,))
    ...
    row = poll_for_row(SALES_DSN, "SELECT id FROM sales WHERE id = %s AND quantity = 42", (sale_id,))
